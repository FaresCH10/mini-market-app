...
export default function ManageProducts() {
  ...
  const handleRemoveBackground = async () => {
    if (!formData.image_url.trim()) {
      toast.error('Set an image URL first')
      return
    }
    setRemovingBackground(true)
    try {
      const res = await fetch('/api/admin/remove-product-background', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      setFormData((prev) => ({ ...prev, image_url: processedImageUrl }))
            ...
            <div>
              ...
              <div className="mt-2 flex items-center gap-2 flex-wrap">
                ...
                <button
                  ...
                  disabled={removingBackground || !formData.image_url.trim()}
                  className="px-3 py-2 rounded-lg border border-emerald-200 text-xs font-semibold text-emerald-700 hover:bg-emerald-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors flex items-center gap-1.5"
                >
                  {removingBackground ? (
                    <>
                      <svg className="w-3.5 h-3.5 animate-spin" fill="none" viewBox="0 0 24 24">
                    ...
                    <>
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m2-2l1.586-1.586a2 2 0 012.828 0L20 14M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                      </svg>
                      Remove Background
                    </>
                  )}
